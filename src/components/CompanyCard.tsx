...
const getScoreColor = (score: number, killSwitch?: boolean) => {
  ...
  if (score >= 60) return 'bg-teal-100 text-teal-800 border-teal-200';
...
const getDataQualityLabel = (q: string) => {
  if (q === 'alto') return { label: 'Verificado', color: 'text-emerald-600' };
  if (q === 'medio') return { label: 'Parcial', color: 'text-yellow-600' };
  return { label: 'Estimado', color: 'text-slate-400' };
};
...
export const CompanyCard: React.FC<CompanyCardProps> = ({ company, isHighlighted }) => {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl shadow-sm border transition-all duration-200 overflow-hidden ${isExpanded ? 'ring-2 ring-slate-300 shadow-md' : 'border-slate-200 hover:border-slate-300 hover:shadow-md'
    ...
    >
      ...
      >
        <div className="flex items-center gap-4 flex-1 min-w-0">
          {/* Score Circle */}
          <div className={`flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center font-black text-xl border-2 ${scoreColorClass}`}>
            {company.kill_switch ? '0' : company.score_final}
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-base font-bold text-slate-900 leading-tight">{company.name}</h3>
            ...
            {/* Data quality indicator — small, no CNPJ */}
            <span className={`text-xs mt-1 inline-flex items-center gap-1 ${dq.color}`}>
              ● {dq.label}
            </span>
          </div>
        </div>

        <div className="flex flex-wrap gap-1.5 sm:justify-end max-w-xs flex-shrink-0">
        ...
        <div className="hidden sm:flex items-center self-center text-slate-400 flex-shrink-0">
      ...
      {isExpanded && (
        <div className="border-t border-slate-100 bg-slate-50/80 p-4 sm:p-6">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-5">
            <Building2 size={12} />
            <span>CNPJ raiz: <span className="font-mono">{company.cnpj_root}</span></span>
          </div>
          ...
          {/* Dimensões E S G — 3 colunas lado a lado */}
          <div className="grid grid-cols-3 gap-3 mb-5">
            {(Object.entries(DIMENSION_COLORS) as [keyof typeof DIMENSION_COLORS, (typeof DIMENSION_COLORS)[keyof typeof DIMENSION_COLORS]][]).map(([key, styles]) => (
              <div key={key} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm text-center">
                <span className="text-[10px] uppercase font-bold text-slate-400 mb-1.5 block tracking-wider">
                  {styles.label}
                </span>
                <div className={`text-2xl font-black ${styles.text} leading-none`}>
                  {company.dimensions[key]}
                </div>
                <div className="text-[10px] text-slate-400 mb-1.5">/100</div>
